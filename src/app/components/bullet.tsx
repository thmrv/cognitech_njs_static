...
export default function Bullet({ title = "placeholder", url = "#", active, type = EBulletType.primary }: TBulletProps, router: any) {
    ...
    return (<Suspense fallback={<Loading/>}>
        <a href={url ?? "#"} className={`bullet ibm-plex block no-underline items-center flex ${active ?? defineActive ? "active" : ""} ${type} ${type != EBulletType.primary ? "text-sm xfsize font-normal secondary" : "text-sm font-normal primary"}`} suppressHydrationWarning>
            {title}
        </a>
    </Suspense>
    )
